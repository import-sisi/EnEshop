...
class BillingProfile(models.Model):
    user        = models.OneToOneField(User, null=True, blank=True, on_delete=models.CASCADE) #user is uniqu but email can ununique
    ...
    def __str__(self):
        return self.email
    '''
    that is the difault card and that the gust card and gust charg
    '''
...
def billing_profile_created_recever(sender, instance, *args, **kwargs):
    if not instance.customer_id and instance.email:
        print("ACTUAL API REQUEST Send to stripe/bratintree")
        customer = stripe.Customer.create(
            email = instance.email
        )
        print(customer)
        instance.customer_id = customer.id
pre_save.connect(billing_profile_created_recever, sender=BillingProfile)
    ...
    def add_new(self, billing_profile, token):
        if token:
            ...
            new_card = self.model(
                ...
                exp_month = stripe_card_response.exp_year,
...
'''
default card jadid ra brabar ba true migzarad va default card ghadimi ra barabar bal False gharar midahad
'''
